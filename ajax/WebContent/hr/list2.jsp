...
<script type="text/javascript">
	function openPopup(event) {
		...
		// 해당 엘리먼트에서 href 속성값을 조회함 <--- "data2.jsp?id=사원아이디"
		var url = event.target.getAttribute('href');
		
		// AJAX 처리 시작
		// XHR 객체 생성
		var xhr = new XMLHttpRequest();
		// XHR에 콜백함수 등록 <--- onreadystatechange 이벤트 발생시 실행할 함수
		// onreadystatechange는 XHR의 readyState값이 변할 때마다 발생하는 이벤트
		...
		// XHR 초기화, url에는 클릭한 <a> 태그의 href 속성에서 조회한 jsp 경로가 들어있다.
		xhr.open("GET", url);
