...
public class DataService {
	...
	// 날짜를 전달받아서 해당 날짜의 사고건수로 사망자 수를 출력하는 기능
	...
	public void printDataStatByMonth() {
		...
		for (Data data : datas) {
			...
			Stat savedStat = stats.get(month);			// Map에서 month를 key로 객체를 조회한다.
			if (savedStat == null) {					// 저장된 Stat 객체가 없으면
				Stat stat = new Stat();					// Stat 객체를 생성한다.
				stat.setDeaths(data.getDeath());		// 사망자 수와 사고건수를 담아서
				stat.setRecords(1);
