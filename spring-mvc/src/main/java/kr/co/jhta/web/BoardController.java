 ...
 * 			- 클래스에 부착할 경우, 해당 클래스의 모든 요청 핸들러 메소드의
 * 			- 매핑URI의 접두사로 추가된다.
 ...
 * 							// 아이디, 비밀번호를 POST 요청방식으로 서버로 보낼 때 요청처리
 * 						}
 * 				consumes
 ...
 * 					required=false로 지정하는 경우 거의 대부분 defaultValue를 사용해서 기본값을 같이 사용된다.
 * 				defaultValue
 * 					요청 파라미터 값이 비어 있을 때 기본 값을 지정한다.
 * 					required=false로 지정된 경우 defaultValue를 사용해서 기본값을
 * 					지정할 수 있다.
 * 					@RequestParam(name="pageNo", required=false, defaultValue="1") int pageNo 
 * 					@ReuqestParam(name="type", required=false, defaultValue="국내도서") String bookType
 */
@Controller
...
public class BoardController {
	...
	public String boardList(Model model) {
		...
		// 조회된 게시글을 Model에 달아서 뷰페이지에 전달하기
